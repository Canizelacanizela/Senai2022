...
function validarPlaca(placa){
    if(placa.length == 7){
        ...
        let modeloNovo = /^[a-zA-Z]{3}[0-9]{1}[a-zA-Z]{2}$/;
        let modeloMoto = /^[a-zA-Z]{3}[0-9]{2}[a-zA-Z]{1}$/;

        if(modeloAntigo.test(placa) || modeloNovo.test(placa) || modeloMoto.test(placa));
     }
     return false;
    }
...
function geraCelular(ddd, qtd) {
    ...
    for(let i = 0; i < qtd; i++)
     numeros.push(ddd + '-9' + (Math.floor(Math.random()*9999-1000)+1000) + "-" + (Math.floor(Math.random()*8999+1000)));
